...
class Client {
     ...
     * Summary: 自动化创建接口a
     * @param ApiaAciCreateRequest $request
     * @return ApiaAciCreateResponse
     */
    public function apiaAciCreate($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return ApiaAciCreateResponse
     */
    public function apiaAciCreateEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return ApiaAciCreateResponse::fromMap($this->doRequest("1.0", "antchain.demosdk.aci.create.apia", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }
